...
async def channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not database_channels:
            # No database channels configured - ignore all
            logger.info(f"⏭️ No database channels configured, ignoring all uploads")
            return
        
        if channel_id not in database_channels:
            # This channel is not in the database list - ignore
            logger.info(f"⏭️ Skipping channel {channel_name} (ID: {channel_id}) - Not in database channels list")
            return
